...
void parIO::xml_pMarker(glsol &sol){

  const std::string fname = "rank_xmls/" + sol.config.xmf2_pMarker_fname + "_" + std::to_string(hila::myrank()) + ".xml";
  ...
  xml_out << "<Grid Name=\"dyGiLa-sim-pMarker\" Type=\"Uniform\">\n"
   ...
   xml_out << "  <Attribute Name=\"phaseMaker\" AttributeType=\"Scalar\" Center=\"Node\">\n"
           << "   <DataItem Format=\"HDF\" DataType=\"Float\" Precision=\"8\" Dimensions=" << "\""
           << dim_0 << " " << dim_1 << " " << dim_2 << "\"" << ">" << "\n"
           << "    domain_" << std::setfill('0') << std::setw(6) << hila::myrank() << ".hdf5:/mesh/fields/phaseMarker/values"
           << "\n"
           << "   </DataItem>\n"
           << "  </Attribute>"
           << "\n" << std::flush;
